(() => {
  ...
  const navTrigger = document.getElementById('mag-nav-trigger');
  const magNavbar = document.getElementById('mag-navbar');
  ...
  const contentRows = document.querySelectorAll('#main-wrapper > .container-xl > .row');
  ...
  const closeMobileNav = () => {
    magNavbar?.classList.remove('nav-open');
    navTrigger?.setAttribute('aria-expanded', 'false');
  };

  const installMobileNavToggle = () => {
    if (!navTrigger || !magNavbar) {
      return;
    }

    navTrigger.addEventListener('click', () => {
      const open = magNavbar.classList.toggle('nav-open');
      navTrigger.setAttribute('aria-expanded', open ? 'true' : 'false');
    });

    document.querySelectorAll('#mag-navbar-links a').forEach((link) => {
      link.addEventListener('click', closeMobileNav);
    });

    document.addEventListener('click', (event) => {
      if (magNavbar.classList.contains('nav-open') && !magNavbar.contains(event.target)) {
        closeMobileNav();
      }
    });
  };
  ...
  const closeSearch = () => {
    ...
    navTrigger?.classList.remove('d-none');
  ...
  const openMobileSearch = () => {
    navTrigger?.classList.add('d-none');
